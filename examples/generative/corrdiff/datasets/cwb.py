...
class _ZarrDataset(DownscalingDataset):
    ...
    def __init__(self, path: str, get_target_normalization=
        ...
        if not (era5_valid.ndim == 2 and cwb_valid.ndim == 1 and cwb_valid.
            shape[0] == era5_valid.shape[0]):
    ...
    def _read_time(self):
        ...
        return cftime.num2date(self.group['time'], units=self.group['time']
            .attrs['units'])
    ...
    def normalize_input(self, x, channels=None):
        ...
        norm = self._select_norm_channels(self.group['era5_center'], self.
            group['era5_scale'], channels)
    ...
    def info(self):
        return {'target_normalization': self.get_target_normalization(self.
            group), 'input_normalization': (self.group['era5_center'][:],
...
class FilterTime(DownscalingDataset):
    ...
    def __init__(self, dataset, filter_fn):
        ...
        self._indices = [i for i, t in enumerate(self._dataset.time()) if
            filter_fn(t)]
...
class ZarrDataset(DownscalingDataset):
    ...
    def __init__(self, dataset, in_channels=(0, 1, 2, 3, 4, 9, 10, 11, 12, 
        17, 18, 19), out_channels=(0, 17, 18, 19), img_shape_x=448,
        img_shape_y=448, roll=False, add_grid=True, ds_factor=1, train=True,
        all_times=False, n_history=0, min_path=None, max_path=None,
        global_means_path=None, global_stds_path=None, normalization='v1'):
        if not all_times:
            self._dataset = FilterTime(dataset, is_2021
                ) if train else FilterTime(dataset, is_2021)
        else:
            self._dataset = dataset
        self.train = train
        ...
        self.global_means_path = to_absolute_path(global_means_path
            ) if global_means_path is not None else None
        self.global_stds_path = to_absolute_path(global_stds_path
            ) if global_stds_path is not None else None
    ...
    def __getitem__(self, idx):
        ...
        reshape_args = (y_roll, self.train, self.n_history, self.
            in_channels, self.out_channels, self.img_shape_x, self.
            img_shape_y, self.min_path, self.max_path, self.
            global_means_path, self.global_stds_path, self.normalization,
            self.roll)
    ...
    def longitude(self):
        ...
        return lon if self.train else lon[..., :self.img_shape_y, :self.
            img_shape_x]

    def latitude(self):
        """Get latitude values from the dataset."""
        lat = self._dataset.latitude()
        return lat if self.train else lat[..., :self.img_shape_y, :self.
            img_shape_x]
    ...
    def normalize_input(self, x):
        """Convert input from physical units to normalized data."""
        x_norm = self._dataset.normalize_input(x[:, :len(self.in_channels)],
            channels=self.in_channels)
        return np.concatenate((x_norm, x[:, self.in_channels:]), axis=1)

    def denormalize_input(self, x):
        """Convert input from normalized data to physical units."""
        x_denorm = self._dataset.denormalize_input(x[:, :len(self.
    ...
    def _create_lowres_(self, x, factor=4):
        x = x.transpose(1, 2, 0)
        x = x[::factor, ::factor, :]
        x = cv2.resize(x, (x.shape[1] * factor, x.shape[0] * factor),
            interpolation=cv2.INTER_CUBIC)
        x = x.transpose(2, 0, 1)
        return x


def get_zarr_dataset(*, data_path, normalization='v1', all_times=False, **
    kwargs):
    """Get a Zarr dataset for training or evaluation."""
    data_path = to_absolute_path(data_path)
    get_target_normalization = {'v1': get_target_normalizations_v1, 'v2':
        get_target_normalizations_v2}[normalization]
    logger.info(f'Normalization: {normalization}')
    zdataset = _ZarrDataset(data_path, get_target_normalization=
        get_target_normalization)
    return ZarrDataset(dataset=zdataset, normalization=normalization,
        all_times=all_times, **kwargs)
