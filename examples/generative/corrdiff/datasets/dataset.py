...
def init_train_valid_datasets_from_config(dataset_cfg: dict, dataloader_cfg:
    Union[dict, None]=None, batch_size: int=1, seed: int=0,
    validation_dataset_cfg: Union[dict, None]=None) ->Tuple[base.
    DownscalingDataset, Iterable, Union[base.DownscalingDataset, None],
    Union[Iterable, None]]:
    """
    ...
    dataset, dataset_iter = init_dataset_from_config(config, dataloader_cfg,
...
def init_dataset_from_config(dataset_cfg: dict, dataloader_cfg: Union[dict,
    None]=None, batch_size: int=1, seed: int=0) ->Tuple[base.
    DownscalingDataset, Iterable]:
    ...
    dataset_sampler = InfiniteSampler(dataset=dataset_obj, rank=dist.rank,
        num_replicas=dist.world_size, seed=seed)
    dataset_iterator = iter(paddle.io.DataLoader(dataset=dataset_obj,
        batch_size=batch_size, worker_init_fn=None,))
