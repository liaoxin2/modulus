...
def get_dataset_and_sampler(dataset_cfg, times):
    ...
    dataset, _ = init_dataset_from_config(dataset_cfg,
        batch_size=1)
    plot_times = [convert_datetime_to_cftime(datetime.datetime.strptime(
        time, '%Y-%m-%dT%H:%M:%S')) for time in times]
...
def save_images(writer, dataset: DownscalingDataset, times,
    image_out, image_tar, image_lr, time_index, t_index):
    """
    ...
    for idx in range(tuple(image_out.shape)[0]):
        ...
        for channel_idx in range(tuple(image_out2.shape)[1]):
            ...
            writer.write_prediction(channel_name, time_index, idx,
                image_out2[0, channel_idx])
