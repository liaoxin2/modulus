    ...
    def __init__(
        ...
        self.weight = (
            self.create_parameter(
                ...
                default_initializer=paddle.nn.initializer.Assign(
                    weight_init(
                        [out_channels, in_channels, kernel, kernel], **init_kwargs
                    )
                    * init_weight
                ),
            )
...
class GroupNorm(paddle.nn.Layer):
    ...
    def forward(self, x):
        if self.training:
            # Use default paddle implementation of GroupNorm for training
            ...
            var = x.var(axis=[2, 3, 4], keepdim=True)
...
class AttentionOp(paddle.autograd.PyLayer):
    ...
    def backward(ctx, dw):
        ...
        # )
        db = dw * w
        db = db - w * db.sum(axis=2, keepdim=True)
        dq = paddle.einsum("nck,nqk->ncq", k.to(paddle.float32), db).to(
            q.dtype
        ) / np.sqrt(k.shape[1])
        dk = paddle.einsum("ncq,nqk->nck", q.to(paddle.float32), db).to(
            k.dtype
        ) / np.sqrt(k.shape[1])
        return dq, dk
...
class UNetBlock(paddle.nn.Layer):
    ...
    def forward(self, x, emb):
        paddle.framework.core.nvprof_nvtx_push("UNetBlock")
        orig = x
        ...
        if self.adaptive_scale:
            ...
            x = silu(paddle.addcmul(shift, self.norm1(x), scale + 1))
        else:
            x = silu(self.norm1(x.add_(params)))

        x = self.conv1(
            paddle.nn.functional.dropout(x, p=self.dropout, training=self.training)
        )
