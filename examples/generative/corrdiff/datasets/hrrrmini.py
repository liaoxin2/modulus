...
class HRRRMiniDataset(DownscalingDataset):
    ...
    def __init__(self, data_path: str, stats_path: str, input_variables:
        Union[List[str], None]=None, output_variables: Union[List[str],
        None]=None, invariant_variables: Union[List[str], None]=(
        'elev_mean', 'lsm_mean')):
        self.input, self.input_variables = _load_dataset(data_path, 'input',
            input_variables)
        self.output, self.output_variables = _load_dataset(data_path,
            'output', output_variables)
        self.invariants, self.invariant_variables = _load_dataset(data_path,
            'invariant', invariant_variables, stack_axis=0)
        ...
        with open(stats_path, 'r') as f:
            stats = json.load(f)
        input_mean, input_std = _load_stats(stats, self.input_variables,
            'input')
        inv_mean, inv_std = _load_stats(stats, self.invariant_variables,
            'invariant')
        self.input_mean = np.concatenate([input_mean, inv_mean], axis=0)
        self.input_std = np.concatenate([input_std, inv_std], axis=0)
        self.output_mean, self.output_std = _load_stats(stats, self.
    ...
    def __getitem__(self, idx):
        ...
        i, j = self.coords[idx]
        inv = self.invariants[:, i:i + self.img_shape[0], j:j + self.
            img_shape[1]]
    ...
    def input_channels(self) ->List[ChannelMetadata]:
        ...
        invariants = [ChannelMetadata(name=v, auxiliary=True) for v in self
            .invariant_variables]
    ...
    def time(self) ->List:
        ...
        datetimes = (datetime.datetime.utcfromtimestamp(t.tolist() / 
            1000000000.0) for t in self.times)
    ...
    def upsample(self, x):
        ...
        y_shape = x.shape[0], x.shape[1] * self.upsample_factor, x.shape[2
