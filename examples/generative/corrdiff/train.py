...
from helpers.train_helpers import set_patch_shape, set_seed, configure_cuda_for_consistent_precision, compute_num_accumulation_rounds, handle_and_clip_gradients, is_time_for_periodic_task
...
def main(cfg: DictConfig) ->None:
    ...
    if dist.rank == 0:
>>>>>>        writer = torch.utils.tensorboard.SummaryWriter(log_dir='tensorboard')
    ...
    data_loader_kwargs = {'pin_memory': True, 'num_workers': cfg.training.
        perf.dataloader_workers, 'prefetch_factor': 2}
    (dataset, dataset_iterator, validation_dataset, validation_dataset_iterator
        ) = (init_train_valid_datasets_from_config(cfg.dataset,
        data_loader_kwargs, batch_size=cfg.training.hp.batch_size_per_gpu,
        seed=0, validation_dataset_cfg=validation_dataset_cfg))
    ...
    model_args = {'img_out_channels': img_out_channels, 'img_resolution':
        list(img_shape), 'use_fp16': fp16}
    standard_model_cfgs = {'regression': {'img_channels': 4,
        'N_grid_channels': 4, 'embedding_type': 'zero'}, 'diffusion': {
        'img_channels': img_out_channels, 'gridtype': 'sinusoidal',
        'N_grid_channels': 4}, 'patched_diffusion': {'img_channels':
        img_out_channels, 'gridtype': 'learnable', 'N_grid_channels': 100}}
    ...
    if cfg.model.name == 'regression':
        model = UNet(img_in_channels=img_in_channels + model_args[
            'N_grid_channels'], **model_args)
    else:
        model = EDMPrecondSR(img_in_channels=img_in_channels + model_args[
            'N_grid_channels'], **model_args)
    out_0 = model.train()
...
>>>>>>        model = torch.nn.parallel.DistributedDataParallel(model, device_ids
            =[dist.local_rank], broadcast_buffers=True, output_device=dist.
            place, find_unused_parameters=dist.find_unused_parameters)
    if hasattr(cfg.training.io, 'regression_checkpoint_path'):
        regression_checkpoint_path = to_absolute_path(cfg.training.io.
    ...
    if cfg.model.name in ('diffusion', 'patched_diffusion'):
        loss_fn = ResLoss(regression_net=regression_net, img_shape_x=
            img_shape[1], img_shape_y=img_shape[0], patch_shape_x=
            patch_shape[1], patch_shape_y=patch_shape[0], patch_num=
            patch_num, hr_mean_conditioning=cfg.model.hr_mean_conditioning)
    elif cfg.model.name == 'regression':
        loss_fn = RegressionLoss()
    optimizer = paddle.optimizer.Adam(parameters=model.parameters(),
        learning_rate=cfg.training.hp.lr, epsilon=1e-08, beta1=[0.9, 0.999]
        [0], beta2=[0.9, 0.999][1], weight_decay=0.0)
    start_time = time.time()
    batch_gpu_total, num_accumulation_rounds = compute_num_accumulation_rounds(
        cfg.training.hp.total_batch_size, cfg.training.hp.
        batch_size_per_gpu, dist.world_size)
    logger0.info(
        f'Using {num_accumulation_rounds} gradient accumulation rounds')
    if dist.world_size > 1:
        paddle.distributed.barrier()
    try:
        cur_nimg = load_checkpoint(path=f'checkpoints_{cfg.model.name}',
            models=model, optimizer=optimizer, device=dist.place)
    except:
        cur_nimg = 0
    ...
    while not done:
        ...
        for _ in range(num_accumulation_rounds):
            ...
            img_clean = img_clean.to(dist.place).to('float32').contiguous()
            img_lr = img_lr.to(dist.place).to('float32').contiguous()
            labels = labels.to(dist.place).contiguous()
            with paddle.amp.auto_cast(dtype=amp_dtype, enable=enable_amp):
                loss = loss_fn(net=model, img_clean=img_clean, img_lr=
                    img_lr, labels=labels, augment_pipe=None)
        ...
        if dist.world_size > 1:
            paddle.distributed.barrier()
            paddle.distributed.all_reduce(tensor=loss_sum, op=paddle.
        ...
        for g in optimizer.param_groups:
            ...
            g['lr'] *= cfg.training.hp.lr_decay ** ((cur_nimg - lr_rampup) //
                5000000.0)
            current_lr = g['lr']
            if dist.rank == 0:
                writer.add_scalar('learning_rate', current_lr, cur_nimg)
        handle_and_clip_gradients(model, grad_clip_threshold=cfg.training.
            hp.grad_clip_threshold)
        optimizer.step()
        ...
        if validation_dataset_iterator is not None:
            valid_loss_accum = 0
            if is_time_for_periodic_task(cur_nimg, cfg.training.io.
                validation_freq, done, cfg.training.hp.total_batch_size,
                dist.rank):
                with paddle.no_grad():
                    for _ in range(cfg.training.io.validation_steps):
                        img_clean_valid, img_lr_valid, labels_valid = next(
                            validation_dataset_iterator)
                        img_clean_valid = img_clean_valid.to(dist.place).to(
                            'float32').contiguous()
                        img_lr_valid = img_lr_valid.to(dist.place).to('float32'
                            ).contiguous()
                        labels_valid = labels_valid.to(dist.place).contiguous()
                        loss_valid = loss_fn(net=model, img_clean=
                            img_clean_valid, img_lr=img_lr_valid, labels=
                            labels_valid, augment_pipe=None)
                        loss_valid = (loss_valid.sum() / batch_gpu_total).cpu(
                            ).item()
                        valid_loss_accum += (loss_valid / cfg.training.io.
                            validation_steps)
                    valid_loss_sum = paddle.to_tensor(data=[
                        valid_loss_accum], place=dist.place)
                    if dist.world_size > 1:
                        paddle.distributed.barrier()
                        paddle.distributed.all_reduce(tensor=valid_loss_sum,
                            op=paddle.distributed.ReduceOp.SUM)
                    average_valid_loss = valid_loss_sum / dist.world_size
                    if dist.rank == 0:
                        writer.add_scalar('validation_loss',
                            average_valid_loss, cur_nimg)
        if is_time_for_periodic_task(cur_nimg, cfg.training.io.
            print_progress_freq, done, cfg.training.hp.total_batch_size,
...
>>>>>>            torch.cuda.reset_peak_memory_stats()
        if dist.world_size > 1:
            paddle.distributed.barrier()
        if is_time_for_periodic_task(cur_nimg, cfg.training.io.
            save_checkpoint_freq, done, cfg.training.hp.total_batch_size,
            dist.rank, rank_0_only=True):
            save_checkpoint(path=f'checkpoints_{cfg.model.name}', models=
                model, optimizer=optimizer, epoch=cur_nimg)
