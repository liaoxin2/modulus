...
def plot_channels(group, time_idx: int):
    ...
    ncols = 4
    fig, axs = plt.subplots(nrows=num_channels // ncols if num_channels %
        ncols == 0 else num_channels // ncols + 1, ncols=ncols, sharex=True,
        sharey=True, constrained_layout=True, figsize=(15, 15))
...
def main(file, output_dir, sample):
    ...
    for idx in range(len(times)):
        ...
        fig, axs = plt.subplots(nrows=len(output_channels), ncols=3, sharex
            =True, sharey=True, constrained_layout=True, figsize=(12, 12))
        row = axs[0]
        ...
        for ch in range(len(output_channels)):
            ...
            def plot_panel(ax, data, **kwargs):
                if channel == 'maximum_radar_reflectivity':
                    return ax.pcolormesh(f['lon'], f['lat'], data, cmap=
                        'magma', vmin=0, vmax=vmax)
                if channel == 'temperature_2m':
                    return ax.pcolormesh(f['lon'], f['lat'], data, cmap=
                        'magma', vmin=vmin, vmax=vmax)
                else:
                    if vmin < 0 < vmax:
